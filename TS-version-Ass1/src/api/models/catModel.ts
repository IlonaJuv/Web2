...
const updateCat = async (
  ...
  if (headers.affectedRows === 0) {
    throw new CustomError('No cats were updated', 400);
  }
  return true;
};
...
const getAllCats = async (): Promise<Cat[]> => {
  ...
  if (rows.length === 0) {
    throw new CustomError('Kitties not found :(', 404);
  }
  const cats: Cat[] = rows.map((row) => ({
    ...row,
    owner: JSON.parse(row.owner?.toString() || '{}'),
...
const getCat = async (catId: number): Promise<Cat> => {
  ...
  if (rows.length === 0) {
    throw new CustomError('Kitty not found :(', 404);
  }
...
const addCat = async (data: PostCat): Promise<number> => {
  try {
    ...
    if (headers.affectedRows === 0) {
      throw new CustomError('No cats were added', 400);
    }
    console.log(headers.info);
    return headers.insertId;
  } catch (error) {
...
/**
 * if role is admin, update anycat
 * if role is user, update only cats owned by user
 */
const updateCat = async (
  data: PutCat,
...
const deleteCat = async (catId: number): Promise<boolean> => {
  ...
  if (headers.affectedRows === 0) {
    throw new CustomError('No cats were deleted', 400);
  }
  return true;
};
